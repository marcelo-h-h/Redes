...
def check_ack_rcv(fd):
    ...
    if dst_port != 7000:
        return 0
    elif (FLAGS_ACK & flags) == FLAGS_ACK and (FLAGS_FIN & flags) != FLAGS_FIN:
        print('ack_received')
        return 1
    elif (FLAGS_FIN & flags) == FLAGS_FIN:
        return 2
    else:
        return 0
...
def send_next(fd, conexao):
    ...
    parando = False
    if check_ack_rcv(fd) == 1:
        payload = conexao.send_queue[:MSS]
        print('Tamanho na fila: %d' % len(conexao.send_queue))
    ...
    elif check_ack_rcv(fd) == 2:
        segment = struct.pack('!HHIIHHHH', src_port, dst_port, conexao.seq_no,
                                     conexao.ack_no, (5<<12)|FLAGS_ACK,
                                     0, 0, 0)
        segment = fix_checksum(segment, src_addr, dst_addr)
        print('FIN Recebido, finalizando conexão')
        fd.sendto(segment, (dst_addr, dst_port))
        parando = True
...
def raw_recv(fd):
    ...
    if (flags & FLAGS_SYN) == FLAGS_SYN and (flags & FLAGS_ACK) != FLAGS_ACK:
